...
from crgeo.dataset.extraction.road_network.types import LaneletEdgeType, TrafficFlowEdgeConnections
...
class BaseRoadNetworkGraph(ABC, nx.DiGraph, AutoReprMixin):
    ...
    def get_torch_data(
        ...
        **kwargs
        ...
        from crgeo.common.torch_utils.helpers import to_float32, to_padded_sequence
        from crgeo.common.torch_utils.pygeo import nx_graph_to_pyg_data
    ...
    def from_data(cls,
        ...
        assert not hasattr(pyg_data, 'batch') or pyg_data.batch.max() == 0
        graph = torch_geometric.utils.convert.to_networkx(data=pyg_data, node_attrs=node_attrs, edge_attrs=edge_attrs)
        return cls(graph=graph)

    def save(self, file_path: str) -> None:
        from crgeo.common.utils.filesystem import save_pickle
        save_pickle(self, file_path=file_path)

    @classmethod
    def load(cls, file_path: str) -> Type[BaseRoadNetworkGraph]:
        from crgeo.common.utils.filesystem import load_pickle
