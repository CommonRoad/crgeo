...
def ft_veh_state(params: VFeatureParams) -> FeatureDict:
    ...
    acceleration = torch.tensor([
        params.state.acceleration if "acceleration" in params.state.attributes else 0.0,
        params.state.acceleration_y if "acceleration_y" in params.state.attributes else 0.0,
    ], dtype=torch.float)
    return {
