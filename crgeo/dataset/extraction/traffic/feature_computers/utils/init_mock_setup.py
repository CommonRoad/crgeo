...
from crgeo.common.io_extensions.mock_objects import create_dummy_lanelet, create_dummy_obstacle, create_dummy_scenario
from commonroad.scenario.lanelet import LaneletNetwork
from crgeo.dataset.extraction.traffic.feature_computers.types import BaseFeatureParams
from crgeo.simulation.base_simulation import BaseSimulationOptions
...
def _init_mock_setup(simulation_options: BaseSimulationOptions) -> Tuple[ScenarioSimulation, List[BaseFeatureParams]]:
    ...
    dummy_lanelet_network = LaneletNetwork.create_from_lanelet_list([dummy_lanelet_1, dummy_lanelet_2], cleanup_ids=True)
    dummy_obstacles = []
    for i in range(2):
    ...
    for state in dummy_states:
        ...
        assert len(set(dummy_scenario.lanelet_network.find_lanelet_by_position([state.position])[0])) > 0, "TrafficExtractor setup: Lanelet assignment for dummy state failed"
