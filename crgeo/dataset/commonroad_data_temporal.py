...
from crgeo.dataset.commonroad_data import CommonRoadData, VirtualAttributesEdgeStorage, VirtualAttributesNodeStorage
...
class CommonRoadDataTemporal(CommonRoadData):
    ...
    def from_data_list(
        ...
        temporal_graph, slice_dict, inc_dict = torch_geometric.data.collate.collate(
            cls,
            data_list=data_list,
            increment=True,
            add_batch=True,
        ...
        *,
        ...
        for time_step in range(1, T):
            ...
            for obstacle_id in obstacle_id_to_obstacle_idx[time_step].keys():
                ...
                params.curr_obstacle_idx = curr_obstacle_idx
                ...
                for ts_offset in range(1, max_ts_offset + 1):
                    ...
                    if past_obstacle_idx is not None:
                        params.past_obstacle_idx = past_obstacle_idx
                        ...
                        if feature_computers is not None:
                            for feature_computer in feature_computers:
                                curr_edge_attr_list += feature_computer(params)
    ...
    def remove_duplicate_lanelet_graphs_(data: CommonRoadDataTemporal) -> None:
        ...
        for key in ["lanelet", ("lanelet", "to", "lanelet")]:
            for attr in data._slice_dict[key].keys():
                data[key][attr] = torch_geometric.data.separate._separate(
                    key=attr,
                    value=data[key][attr],
                    idx=0,
    ...
    try:
        index_select = torch_geometric.data.Batch.index_select
        to_data_list = torch_geometric.data.Batch.to_data_list
        num_graphs = torch_geometric.data.Batch.num_graphs
    except NameError:
        pass
    # exclude __reduce__
    ...
    def get_example(self, idx: int) -> CommonRoadData:
        import torch_geometric.data.separate
        data = cast(CommonRoadData, torch_geometric.data.separate(
            cls=CommonRoadData,
            batch=self,
            idx=idx,
    ...
    def __getitem__(self, idx: Union[torch_geometric.typing.NodeType, torch_geometric.typing.EdgeType, int, np.integer, torch_geometric.data.dataset.IndexType]) -> Any:
        ...
        return torch_geometric.data.Batch.__getitem__(self, idx)
    ...
    def storage_attribute_at_time_step(
        ...
        import torch_geometric.data.separate
        return torch_geometric.data.separate._separate(
            key=key,
            value=self[key][attr],
            idx=time_step,
    ...
    def get_node_features_temporal_sequence_for_vehicle(self, vehicle_id: int, keys: Optional[Sequence[str]] = None) -> torch.Tensor:
        ...
        x_v: torch.Tensor = x[id==vehicle_id, :]
        return x_v

    def get_node_features_temporal_sequence(self, keys: Optional[Sequence[str]] = None) -> torch.Tensor:
        # TODO: Document, rename & unit-test
        if self.vehicle is None:
            raise AttributeError("self.vehicle is None")
        if keys is None:
        ...
        from crgeo.common.torch_utils.helpers import get_index_mapping_by_first_occurrence
