...
NormalizationParams = Tuple[torch.Tensor, torch.Tensor]
...
class FeatureNormalizer:
    ...
    def is_feature(data: T_CommonRoadData, store: str, key: str) -> bool:
        ...
        is_feature = isinstance(tensor, torch.Tensor) and torch.is_floating_point(tensor) and tensor.ndim <= 2 and tensor.size(-1) > 0
        return is_feature
