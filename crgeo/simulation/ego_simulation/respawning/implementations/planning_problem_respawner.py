...
from crgeo.simulation.ego_simulation.respawning.base_respawner import BaseRespawner, BaseRespawnerOptions, RespawnerSetupFailure, T_Respawn_Tuple
...
class PlanningProblemRespawner(BaseRespawner):
    ...
    def _setup(self, ego_vehicle_simulation: EgoVehicleSimulation) -> None:
        ...
        if planning_problem is None:
            raise RespawnerSetupFailure(f"PlanningProblemRespawner needs a planning problem, which it appears is lacking for the current scenario {ego_vehicle_simulation.current_scenario.scenario_id}. Consider using the RandomRespawner instead.")
        ...
        self._goal_position = self._goal_state.position.shapes[0] if isinstance(self._goal_state.position, ShapeGroup) else self._goal_state.position  # Note: position itself is a shape

        lanelet_network = ego_vehicle_simulation.current_scenario.lanelet_network
        goal_lanelet_assignment_position = ego_vehicle_simulation.current_scenario.lanelet_network.find_lanelet_by_position([self._goal_position.center])

        goal_lanelet_id = None
        # Fallback if assignment by position fails, happens in edge case where ego vehicle exits into missing lanelet or in crowded intersection
        if goal_lanelet_assignment_position == [[]]:
            lanelet_assignment_shape = ego_vehicle_simulation.current_scenario.lanelet_network.find_lanelet_by_shape(self._goal_position)
            ...
            if len(lanelet_assignment_shape) > 1:
                ...
                close_lanelets = {lanelet.lanelet_id for lanelet in lanelet_network.lanelets_in_proximity(self._goal_position.center, radius=1.0)}
        ...
        if goal_lanelet_id is None:
            raise RespawnerSetupFailure(f"Failed to assign goal lanelet for scenario {str(ego_vehicle_simulation.current_scenario.scenario_id)}")
