...
def to_float32_tensors(ndarrays: List[np.ndarray]) -> List[torch.Tensor]:
    return [
...
def to_padded_sequence(ndarrays: List[np.ndarray]) -> torch.Tensor:
...
def optimizer_to(optim: optim.optimizer.Optimizer, device: torch.device) -> None:
...
def flatten_data(
    ...
    Returns:
        Dict[str, torch.Tensor]: Dictionary of fixed-size PyTorch tensors.
    """
    ...
    for k_str, v in tensors.items():
        ...
        if k[-1] == 'edge_index':
            ...
            v_padded = torch.nn.functional.pad(v, (0, padding - v.shape[1], 0, 0))
            ...
            if v.shape[1] == 0:
                ...
                v_padded = torch.nn.functional.pad(v, (0, 0, 0, padding - v.shape[0]))
...
def reconstruct_data(
    ...
    Args:
        tensors (Dict[str, torch.Tensor]): Dictionary of tensors to be converted to HeteroData instance.
    ...
    reconstructed_data = torch_geometric.data.HeteroData(reconstructed_dict)
...
def assert_size(t: torch.Tensor, size: Tuple[Union[int, None], ...]) -> None:
    if t.ndim != len(size):
