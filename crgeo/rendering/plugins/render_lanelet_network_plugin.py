...
class RenderLaneletNetworkPlugin(BaseRendererPlugin):
    def __init__(
        ...
        lanelet_color: T_ColorTuple = (0.4, 0.4, 0.4, 1.0), # grey
    ...
    def _render_from_graph(self, viewer: Viewer2D, params: RenderParams) -> None:
        ...
        if isinstance(left_vertices, torch.Tensor) and left_vertices.dim() == 2:
...
def draw_lanelet(
    ...
    if fill_color is not None:
        left_polyline = left_vertices if isinstance(left_vertices, ContinuousPolyline) else ContinuousPolyline(left_vertices)
        center_polyline = center_vertices if isinstance(center_vertices, ContinuousPolyline) else ContinuousPolyline(center_vertices)
        right_polyline = right_vertices if isinstance(right_vertices, ContinuousPolyline) else ContinuousPolyline(right_vertices)
