...
class CR2SumoMapConverter(AbstractScenarioWrapper):
    ...
    def __init__(self,
        ...
        # NETCONVERT files
        self._nodes_file = Path("")
        self._edges_file = Path("")
        self._connections_file = Path("")
        self._traffic_file = Path("")
        self._additional_file = Path("")
        # path to SUMO typ.xml file
        self._type_file = Path("")
        self._output_file = Path("")

        # simulation params
        self.scenario_name = ""
        self.sumo_cfg_file = Path("")
    ...
    def _write_nodes_file(self, output_path: Path) -> Path:
        """
        Functio for writing the nodes file
        :param output_path: path for the file
        :return: nothing
        """
        file_path = output_path.parent.joinpath(f"{self.conf.scenario_name}.nod.xml")
    ...
    def _write_edges_file(self, output_path: Path) -> Path:
        """
        Function for writing the edges file
        :param output_path: path for the file
        :return: nothing
        """
        file_path = output_path.parent.joinpath(f"{self.conf.scenario_name}.edg.xml")
        with open(file_path, 'w+') as output_file:
            sumolib.writeXMLHeader(output_file, 'CommonRoad Scenario Designer')
    ...
    def _write_connections_file(self, output_path: Path) -> Path:
        ...
        :return: nothing
        """
        file_path = output_path.parent.joinpath(f"{self.conf.scenario_name}.con.xml")
        with open(file_path, 'w+') as output_file:
            sumolib.writeXMLHeader(output_file, 'CommonRoad Scenario Designer')
    ...
    def _write_traffic_file(self, output_path: Path) -> Path:
        ...
        :param output_path: path for the file
        """
        file_path = output_path.parent.joinpath(f"{self.conf.scenario_name}.tll.xml")
    ...
    def _write_edge_type_file(self, output_path: Path) -> Path:
        ...
        :param output_path: path for the file
        """
        file_path = output_path.parent.joinpath(f"{self.conf.scenario_name}.typ.xml")
        with open(file_path, "w+") as f:
            sumolib.writeXMLHeader(f, 'CommonRoad Scenario Designer')
    ...
    def create_sumo_files(self, output_folder: Path, traffic_from_trajectories=False, cleanup_tmp_files=True) -> bool:
        ...
        """
        output_path = output_folder.joinpath(self.conf.scenario_name + '.net.xml')
    ...
    def _create_routes_from_trajectories(self, scenario: Scenario, output_folder: Path) -> bool:
        ...
        net_file = output_folder.joinpath(scenario_name + '.net.xml')
    ...
    def _create_random_routes(self, net_file: Path, scenario_name: Path = None, return_files=True) -> bool:
        """
    ...
    def _generate_add_file(self, scenario_name: Path,
        ...
        add_file = output_folder.joinpath(scenario_name + '.add.xml')
    ...
    def _generate_rou_file(
        ...
        route_files: Dict[str, Path] = {
            'vehicle': out_folder.joinpath(scenario_name + ".vehicles.rou.xml"),
        }
        trip_files: Dict[str, Path] = {
            'vehicle': out_folder.joinpath(scenario_name + '.vehicles.trips.xml'),
        }

        if self.conf.veh_distribution[ObstacleType.PEDESTRIAN] > 0:
            route_files["pedestrian"] = out_folder.joinpath(scenario_name + ".pedestrians.rou.xml")
            trip_files["pedestrian"] = out_folder.joinpath(scenario_name + '.pedestrian.trips.xml')
    ...
    def generate_cfg_file(scenario_name: Path, net_file: Path,
        ...
        sumo_cfg_file = output_folder.joinpath(scenario_name+ '.sumo.cfg')
        tree = ET.parse(Path(__file__).parent.joinpath(DEFAULT_CFG_FILE))
