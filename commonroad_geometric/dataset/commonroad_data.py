...
from torch_geometric.data.view import ItemsView, KeysView, MappingView, ValuesView

from commonroad_geometric.common.torch_utils.helpers import flatten_data, reconstruct_data
...
class VirtualAttributesBaseStorage(BaseStorage):
    ...
    def keys(self, *args: List[str], virtual_attributes: bool = False) -> Union[KeysView, CustomKeysView]:
        if not virtual_attributes:
            return super().keys(*args)
        return CustomKeysView(self, *args)

    def values(self, *args: List[str], virtual_attributes: bool = False) -> Union[ValuesView, CustomValuesView]:
        if not virtual_attributes:
            return super().values(*args)
        return CustomValuesView(self, *args)

    def items(self, *args: List[str], virtual_attributes: bool = False) -> Union[ItemsView, CustomItemsView]:
        if not virtual_attributes:
    ...
    def __repr__(self) -> str:
        ...
        for k, v in self.items():
            if isinstance(v, Tensor):
                type_string = f"Tensor ({v.shape}, {v.dtype})"
