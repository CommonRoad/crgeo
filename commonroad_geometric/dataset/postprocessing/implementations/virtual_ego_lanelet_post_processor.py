...
class VirtualEgoLaneletPostProcessor(BaseDataPostprocessor):
    ...
    def __call__(
        ...
        if len(start_left_polyline_cut) == 2 and len(start_center_polyline_cut) == 2 and len(start_right_polyline_cut) == 2:
            ...
            start_lanelet_left_vertices_new = pad(start_lanelet_left_vertices_new, (0, padding_size - start_lanelet_left_vertices_new.shape[0]))
            start_lanelet_center_vertices_new = torch.from_numpy(start_center_polyline_cut[0].flatten())
            start_lanelet_center_vertices_new = pad(start_lanelet_center_vertices_new, (0, padding_size - start_lanelet_center_vertices_new.shape[0]))
            start_lanelet_right_vertices_new = torch.from_numpy(start_right_polyline_cut[0].flatten())
            start_lanelet_right_vertices_new = pad(start_lanelet_right_vertices_new, (0, padding_size - start_lanelet_right_vertices_new.shape[0]))
        ...
        if has_end_lanelet:
            ...
            if len(end_left_polyline_cut) == 2 and len(end_center_polyline_cut) == 2 and len(end_right_polyline_cut) == 2:
                ...
                end_lanelet_left_vertices_new = pad(end_lanelet_left_vertices_new, (0, padding_size - end_lanelet_left_vertices_new.shape[0]))
                end_lanelet_center_vertices_new = torch.from_numpy(end_center_polyline_cut[1].flatten())
                end_lanelet_center_vertices_new = pad(end_lanelet_center_vertices_new, (0, padding_size - end_lanelet_center_vertices_new.shape[0]))
                end_lanelet_right_vertices_new = torch.from_numpy(end_right_polyline_cut[1].flatten())
                end_lanelet_right_vertices_new = pad(end_lanelet_right_vertices_new, (0, padding_size - end_lanelet_right_vertices_new.shape[0]))
        ...
        data.lanelet.left_vertices = torch.cat([data.lanelet.left_vertices, pad(virtual_lanelet_left_vertices_new, (0, padding_size - virtual_lanelet_left_vertices_new.shape[1]))], 0)
        data.lanelet.center_vertices = torch.cat([data.lanelet.center_vertices, pad(virtual_lanelet_center_vertices_new, (0, padding_size - virtual_lanelet_center_vertices_new.shape[1]))], 0)
        data.lanelet.right_vertices = torch.cat([data.lanelet.right_vertices, pad(virtual_lanelet_right_vertices_new, (0, padding_size - virtual_lanelet_right_vertices_new.shape[1]))], 0)
