...
class SumoSimulation(BaseSimulation):
    ...
    def current_time_step(self, time_step_to_jump_to: int) -> None:
        raise SimulationRuntimeError(
            f"BaseSimulation.current_time_step should never be set directly for {type(self).__name__}.")
    ...
    def _step(
        ...
        self._time_step_to_obstacles[self.current_time_step] = get_dynamic_obstacles_at_timestep(
            self.current_scenario, self.current_time_step)
    ...
    def _spawn_ego_vehicle(self, ego_vehicle: EgoVehicle) -> None:
        ...
        # init_ego_vehicles_from_planning_problem calls presimulation_silent(1),
        # which only works at _CRSumoSimulation.current_time_step == 0
    ...
    def presimulation_silent(self, pre_simulation_steps: Union[int, Literal['auto']]) -> int:
        ...
        def presimulation_step() -> Scenario:
            ...
            steps += 1
            if self._options.report_presimulation_progress:
        ...
        if isinstance(pre_simulation_steps, int):
            ...
            for _ in range(MAX_AUTO_PRESIMULATION_STEPS):
                ...
                raise PreSimulationStepLimitExceededError(
                    f"Exceeded limit of {MAX_AUTO_PRESIMULATION_STEPS} pre-simulation steps")
