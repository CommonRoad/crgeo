...
class SumoInitializationConfig():
    ...
    id_dict: Dict[str, int] = field(default_factory=lambda: {
                                    'obstacleVehicle': 3, 'egoVehicle': 4, 'obstaclePedestrian': 5})
    ...
    # ego vehicle
    # _cr_sumo_simulation.init_ego_vehicle(), _cr_sumo_simulation._fetch_sumo_vehicles()
    ...
    # _cr_sumo_simulation.check_lanelets_future_change()
    lanelet_check_time_window: int = int(2 / dt)
    # The absolute margin allowed between the planner position and ego position in SUMO
    # _cr_sumo_simulation._consistency_protection()
    protection_margin: float = 2.0
    ...
    # _cr_sumo_simulation._check_sync_mechanism()
    lane_change_sync: bool = False
    # tolerance for detecting start of lane change
    # _cr_sumo_simulation._check_lc_start()
    lane_change_tol: float = 0.00
    ...
    # TODO: replace with sumo_comandline_config.seed
    random_seed: Optional[int] = cl_conf.seed
