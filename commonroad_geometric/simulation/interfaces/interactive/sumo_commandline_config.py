...
from commonroad_geometric.external.sumocr.sumo_config.pathConfig import SUMO_BINARY
...
class SumoCommandLineConfig:
    ...
    """
    _sumo_binary: Optional[str] = "sumo"
    ...
    fcd_output__filter_shapes: Optional[List[str]] = None
    ...
    def _get_valid_attr_list(self) -> List[Tuple[str, str]]:
        ...
        """
        def transform_tuple(tpl): return (
            _name_to_flag(tpl[0]),
    ...
    def save_config(self):
        ...
        """
        output_dir = Path(__file__).parent / "config_files"
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / "sumo_saved_config.xml"
        cmd = self.generate_command()
        cmd.extend(['--save-configuration', str(output_path)])
        subprocess.run(cmd)
