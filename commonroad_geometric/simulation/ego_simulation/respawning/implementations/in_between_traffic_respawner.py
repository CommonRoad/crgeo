...
from commonroad.scenario.state import State, InitialState, CustomState
...
class InBetweenTrafficRespawner(BaseRespawner):
    ...
    def _get_respawn_tuple(
            self, ego_vehicle_simulation: EgoVehicleSimulation) -> Tuple[State, np.ndarray, Lanelet, bool]:
        ...
        try:
            while attempts_outer < self._options.max_attempts_outer and len(unique_list) > 0:
                ...
                while not has_multiple_vehicles and len(unique_list) > 0:
                    ...
                    obstacles_on_start_lanelet = ego_vehicle_simulation.simulation.get_obstacles_on_lanelet(
                        route_start_lanelet_id)
                ...
                arcLengths = sorted([center_polyline.get_projected_arclength(state_at_time(
                ...
                if self._options.random_goal_arclength:
                    goal_arclength = min(
                        goal_lanelet.distance[-1] - self._options.goal_region_length / 2, random.random() * goal_lanelet.distance[-1])
                else:
                ...
                if max_arc_length_diff > self._options.min_threshold_diff:
                    filtered_list = list(filter(lambda x: x is not None, [
                                         i if arcLengths[i + 1] - arcLengths[i] > self._options.min_threshold_diff else None for i in range(len(arcLengths) - 1)]))
                    ...
                    start_arclength = start_arclength + \
                        self._options.start_arclength_offset + random.randrange(0, randlimit)
        ...
        if start_position is None:
            if len(retained_arclength_diff) > 0:
                ...
                filtered_list = list(filter(lambda x: x is not None, [
                                     i if retained_arclength_diff[key][i + 1] - retained_arclength_diff[key][i] >= 10 else None for i in range(len(retained_arclength_diff[key]) - 1)]))
