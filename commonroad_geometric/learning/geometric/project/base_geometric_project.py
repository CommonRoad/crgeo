...
from commonroad_geometric.dataset.commonroad_dataset import CommonRoadDataset, T_Data
...
from commonroad_geometric.learning.geometric.base_geometric import BaseGeometric, MODEL_FILE, MODEL_FILENAME, MODEL_FILETYPE, OPTIMIZER_FILE
...
class GeometricProjectDirectoryStructure:
    ...
    def get_model_checkpoint_dir(self, checkpoint: Optional[str]) -> Path:
...
class BaseGeometricProject(BaseProject):
    ...
    def __init__(
        ...
        self._model_cls: Optional[Type][BaseGeometric] = None
        ...
        self.project_name = f"GeometricProject_{type(self).__name__}"

        super().__init__()

    @property
    def model_cls(self) -> Type[BaseGeometric]:
        if self._model_cls is None:
            self._model_cls = self.configure_model(self.model_cfg, self.experiment)
    ...
    def render_latest(self) -> None:
        ...
        logger.info(f"Rendering latest model: {model_path}")
        render_model(
            model=model_path,
            experiment=self.experiment,
            scenario_path=self.dir_structure.val_scenario_dir,
    ...
    def train(self) -> None:
        ...
        wandb_service = WandbService(
            disable=not self.cfg.wandb_logging,
        ...
        model_dir = self.dir_structure.model_dir / experiment_name
        trainer.launch_trainer(
            ...
            callbacks_computers=self.configure_training_callbacks(wandb_service, model_dir=model_dir),
    ...
    def info(self) -> None:
        dataset = self.get_train_dataset(force_dataset_overwrite=False)
        dataset_info = dataset.get_meta_info()

        # Use pprint to print the dataset information nicely
