...
class KinematicSingleTrackVehicleStates(NamedTuple):
    ...
    steering_angle: Tensor
    orientation: Tensor
    length_wheel_base: Tensor

    @staticmethod
    def num_state_dims() -> int:
        return 6  # length_wheel_base not included
...
class KinematicSingleTrackVehicleModel(VehicleModel[KinematicSingleTrackVehicleStates]):
    ...
    def __init__(
        ...
        steering_angle_delta_bound: float, # symmetric bound
        ...
        assert steering_angle_delta_bound > 0.
        assert steering_angle_bound > 0.
        self.velocity_bounds = velocity_bounds
        self.acceleration_bounds = acceleration_bounds
        self.steering_angle_delta_bound = steering_angle_delta_bound 
    ...
    def compute_next_state(
        ...
        steering_angle_delta = self.steering_angle_delta_bound * torch.tanh(input[:, 1:2])
        steering_angle = states.steering_angle + dt * steering_angle_delta
        steering_angle = torch.clamp(steering_angle, min=-self.steering_angle_bound, max=self.steering_angle_bound)
