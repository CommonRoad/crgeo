...
class OccupancyDecoder(BaseModel):
    ...
    def compute_loss(self, data: Union[CommonRoadData, CommonRoadDataTemporal], prediction: Tensor, only_ego: bool = False) -> Tensor:
        if isinstance(prediction, tuple):
            ...
            occupancy = data.v[self.target_attribute].index_select(
                0, sample_ind) if sample_ind is not None else data.v[self.target_attribute]
        else:
            occupancy = data.v[self.target_attribute]

        if only_ego:
            is_ego_mask = data.v.is_ego_mask.bool().squeeze(-1)
            occupancy = occupancy[is_ego_mask, :]

        N = prediction.shape[0]
        prediction_size = self.cfg.prediction_size
        occupancy = occupancy.view(N, prediction_size, prediction_size)
        assert_size(prediction, (N, prediction_size ** 2))

        vehicle_filter = (data.v.type == 0).squeeze(-1)
        if only_ego:
            vehicle_filter = torch.ones(N, dtype=torch.bool, device=occupancy.device)
        
        # enable this for debugging
        # import matplotlib.pyplot as plt
        # indices = torch.where(vehicle_filter)[0][:5]  # Get first 5 indices that meet the condition
        # # Create a figure with 5 rows and 2 columns
        # fig, axes = plt.subplots(nrows=5, ncols=2, figsize=(12, 16))
        # for i, idx in enumerate(indices):
        #     # 'Raw' image subplot
        #     axes[i, 0].imshow(occupancy[idx, :, :].detach().cpu().numpy())
        ...
        true_occupancy = occupancy.view(N, prediction_size ** 2)
        loss = F.binary_cross_entropy(input=prediction[vehicle_filter], target=true_occupancy[vehicle_filter], reduction="mean")
        return loss
    ...
    @staticmethod
    def thresholded_occupancy(
        occupancy: Tensor,
        prediction: Tensor,
    ) -> Tuple[BoolTensor, BoolTensor]:
        N, prediction_size, _ = occupancy.size()
        assert_size(occupancy, (N, prediction_size, prediction_size))
        assert_size(prediction, (N, prediction_size ** 2))
        true_occupancy = occupancy.view(-1, prediction_size ** 2)
        true_occupancy_tresh = true_occupancy > 0.5
        prediction_thresh = prediction > 0.5
        return true_occupancy_tresh, prediction_thresh
    ...
    def compute_binary_loss(
        ...
        true_occupancy_thresh, prediction_thresh = OccupancyDecoder.thresholded_occupancy(
            occupancy=occupancy, prediction=prediction,
        )
        return F.binary_cross_entropy(
            input=prediction_thresh.type(torch.float32),
            target=true_occupancy_thresh.type(torch.float32),
            reduction="mean",
        )
    ...
    def thresholded_prediction_confusion_matrix(
        ...
        actual_pos, actual_neg = true_occupancy_thresh, ~true_occupancy_thresh
