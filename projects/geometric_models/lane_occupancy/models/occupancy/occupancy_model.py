...
class OccupancyModel(BaseGeometric):
    ...
    def __init__(
        self,
        config: Optional[Union[OccupancyModelConfig, dict]] = None,
    ):
        if config is None:
            config = OccupancyModelConfig()
        elif not isinstance(config, OccupancyModelConfig):
            config = OccupancyModelConfig(**config)
    ...
    def configure_renderer_plugins(cls) -> Optional[List[BaseRenderPlugin]]:
        plugins = [
            ...
            # RenderPathOccupancyPredictionPlugin(), # TODO auto set
    ...
    def _build(self, data: CommonRoadData, trial) -> None:
        self.config = OccupancyModelConfig()  # TODO
        if self.config.loss is None:
            self.config.loss = OccupancyLossConfig()
        ...
        if not self.training:
            ...
            if self.config.path_conditioning:
                ...
                if data.walks.max().item() >= data.lanelet.num_nodes:
                    data = self.train_preprocess(data)
                if data.lanelet.batch[data.walks].max().item() + 1 != data.batch_size:
                    data = self.train_preprocess(data)
    ...
    def compute_loss(
        ...
        **kwargs
        ...
        if self.config.path_conditioning:
            ...
            assert data.v.batch.max().item() + 1 == z_ego_route.shape[0]
        else:
            if self.config.variational:
        ...
        def compute_integrals(
            ...
            if path_conditioning:
                ...
                local_integration_domains = bounds_low_abs_offset.unsqueeze(-1)
                local_integration_domains += local_integration_lengths.unsqueeze(-1)
                local_integration_domains = local_integration_domains.expand(-1, -1, -1, self.config.loss.integral_resolution).clone()
                local_integration_domains *= torch.linspace(0, 1, self.config.loss.integral_resolution, device=device)[None, None, None, :] 
                ...
                local_integration_domains = bounds_low.unsqueeze(-1)
                local_integration_domains += local_integration_lengths.unsqueeze(-1)
                local_integration_domains = local_integration_domains.expand(-1, -1, -1, self.config.loss.integral_resolution).clone()
                local_integration_domains *= torch.linspace(0, 1, res, device=device)[None, None, None, :]
